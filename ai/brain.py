...
class Brain:
	...
	def __init__(self, owner, init_state=None):
		log.debug("Initiated brain with owner \"{}\"".format(owner))
		self.owner = owner
		self.stack = deque()
	...
	def __repr__(self):
		if self.state:
			return "<\"{0}\"'s brain in \"{1}\" state>".format(
	...
	def push(self, state):
		if state in self.avail_states:
			...
			log.info("Current state is \"{}\"".format(self.state.name))
			self.state.on_enter()
		else:
			log.error(
				"\"{}\" is not a valid state (not registered).".format(state)
			)
	...
	def pop(self):
		try:
			...
			log.error("Attempted to pop on an empty stack.")
		else:
			s.on_exit()
			log.debug("Removed state \"{}\" from stack.".format(s.name))
			if self.state:
				log.info("Current state is \"{}\"".format(self.state.name))
